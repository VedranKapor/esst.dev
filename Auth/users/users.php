	...
	switch($_POST['action']){
		...
		case 'getAccess': //update case desc
			try{
                ...
                if (isset($_SESSION['us']) && isset($_SESSION['gr']) && $_SESSION['gr'] == 'admin'){
                    ...
                    echo json_encode(array('msg'=>'Admin', "type"=>"ADMINACCESS", "session"=> $_SESSION));
                }
                else if (isset($_SESSION['us']) && isset($_SESSION['gr'])){
                    //echo $_SESSION['us'].'|'.$_SESSION['gr'];
                    echo json_encode(array('msg'=>'User', "type"=>"USERACCESS", "session"=> $_SESSION));
                }
                else{
                    //echo "-1";
		...
		case 'addUser': //update case desc
            try{
                ...
                if ($valid) {
                    ...
                    if (file_put_contents($url, $us)) {
                        mkdir(STORAGE_PATH.$username, 0777);
                        echo json_encode(array('msg'=>'User added!', "type"=>"SUCCESS", "data"=>$users));
                    }
                } else {
                    echo json_encode(array('msg'=>$error, "type"=>"ERROR"));
                }
        ...
        case 'deleteUser':
            try{
                ...
                if(file_exists(STORAGE_PATH.$username))
                    deleteDir(STORAGE_PATH.$username);
                if (file_put_contents($url, $us)) {
                    echo json_encode(array('msg'=>"User deleted!", "type"=>"SUCCESS", "data"=>$users));
                }
            }
            catch(runtimeException $e){
                echo json_encode(array('msg'=>$ex->getMessage(), "type"=>"ERROR"));
